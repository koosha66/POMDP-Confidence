...
class POMDPWithCost(ContinuousPOMDP):
    def __init__(self, k, mu_i, std_i, all_cohs, cost, time_max = 900, std_z = 1.0, step_time = 10.0, num_samples = 20000):
        super(POMDPWithCost, self).__init__(k, mu_i, std_i, std_z, step_time, num_samples)
    ...
    def generateObservations(self, mu):
        file_name = 'observations/fixed_' + str(mu) + '_' + str(self.cost*1000) + '_' + str(self.num_samples) + '.npy'
        try: 
            ...
            for t in range(1, self.t_max):
                Z_t[:, t] = Z_t[:, t - 1] + observe * np.random.normal(mu, 1.00, self.num_samples)
