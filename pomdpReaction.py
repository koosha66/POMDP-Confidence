...
class POMDPReaction(POMDPWithCost):
    def __init__(self, k, mu_i, std_i, all_cohs, cost, time_max = 3000, std_z = 1.0, step_time = 10.0, num_samples = 20000):
        super(POMDPReaction, self).__init__(k, mu_i, std_i, all_cohs, cost, time_max, std_z, step_time, num_samples)

    def generateObservations(self, mu):
        cost = self.cost
        file_name = 'observations/' + str(mu)  + '_' + str(int(self.std_i*100)) + '_' + str(int(self.std_z*100))  + '_' +  str(self.cost*1000) + '_' + str(self.t_max) + str(self.num_samples) + '.npy'
        try: 
