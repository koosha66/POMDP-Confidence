...
class POMDPReactionSeqSz(POMDPReactionSz):
    def __init__(self, k, mu_i, sz, std_i, all_cohs, cost, time_max = 3000, time_delay = 200, std_z = 1.0, step_time = 10.0, num_samples = 20000):
        super(POMDPReactionSeqSz, self).__init__(k, mu_i, sz, std_i, all_cohs, cost, time_max, std_z, step_time, num_samples)
    ...
    def calcDirectionPositiveProb(self, c, t, Z):
        ...
        if Z_cost.size == 0:
            return Z_cost
        Z_cost = Z_cost + np.random.normal(mu * self.t_delay, self.sz * (self.t_delay ** .5), Z_cost.size)
        #Z_cost = (np.sign(Z_cost) == np.sign(Z_cost2)) * Z_cost2 + 0.000000001 * np.sign(Z_cost)
